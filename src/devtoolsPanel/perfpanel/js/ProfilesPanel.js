...
WebInspector.ProfileType.prototype = {
    ...
    getProfile: function(uid) {
        for (var i = 0; i < this._profiles.length; ++i) {
            if (this._profiles[i].uid === uid) return this._profiles[i];
        }
        return null;
    },
    ...
    _clearTempStorage: function() {
        for (var i = 0; i < this._profiles.length; ++i) this._profiles[i].removeTempFile();
    },
    ...
    _reset: function() {
        ...
        for (var i = 0; i < profiles.length; ++i) this._disposeProfile(profiles[i]);
        this._profiles = [];
        this._nextProfileUid = 1;
    },
...
WebInspector.ProfileHeader.prototype = {
    ...
    updateStatus: function(subtitle, wait) {
        this.dispatchEventToListeners(
            WebInspector.ProfileHeader.Events.UpdateStatus,
            new WebInspector.ProfileHeader.StatusUpdate(subtitle, wait)
        );
    },
    ...
    removeTempFile: function() {
        if (this._tempFile) this._tempFile.remove();
    },
    dispose: function() {},
...
WebInspector.ProfilesPanel = function() {
    ...
    this.clearResultsButton = new WebInspector.StatusBarButton(
        WebInspector.UIString('Clear all profiles.'),
    ...
    this._launcherView.addEventListener(
        ...
        this._onProfileTypeSelected,
        this
    );
    this._profileToView = [];
    this._typeIdToSidebarSection = {};
    var types = WebInspector.ProfileTypeRegistry.instance.profileTypes();
    for (var i = 0; i < types.length; i++) this._registerProfileType(types[i]);
...
WebInspector.ProfileTypeRegistry = function() {
    ...
    if (
        Capabilities.isMainFrontend &&
        WebInspector.experimentsSettings.canvasInspection.isEnabled()
    ) {
...
WebInspector.ProfilesPanel.prototype = {
    ...
    _createFileSelectorElement: function() {
        if (this._fileSelectorElement) this.element.removeChild(this._fileSelectorElement);
        this._fileSelectorElement = WebInspector.createFileSelectorElement(
            this._loadFromFile.bind(this)
        );
    ...
    _findProfileTypeByExtension: function(fileName) {
        ...
        for (var i = 0; i < types.length; i++) {
            ...
            if (fileName.endsWith(type.fileExtension())) return type;
        }
        return null;
    },
    _registerShortcuts: function() {
        this.registerShortcuts(
            ...
            this.toggleRecordButton.bind(this)
        );
    },
    _configureCpuProfilerSamplingInterval: function() {
        ...
        function didChangeInterval(error) {
            if (error) WebInspector.console.showErrorMessage(error);
        }
    },
    _loadFromFile: function(file) {
        ...
        if (!profileType) {
            ...
            WebInspector.console.log(
                WebInspector.UIString(
                    "Can't load file. Only files with extensions '%s' can be loaded.",
                    extensions.join("', '")
                )
            );
            return;
        ...
        if (!!profileType.profileBeingRecorded()) {
            WebInspector.console.log(
                WebInspector.UIString("Can't load profile when other profile is recording.")
            );
            return;
        }
    ...
    toggleRecordButton: function() {
        ...
        if (isProfiling) {
            ...
            if (type.hasTemporaryView()) this.showProfile(type.profileBeingRecorded());
        } else {
    ...
    _reset: function() {
        ...
        var types = WebInspector.ProfileTypeRegistry.instance.profileTypes();
        for (var i = 0; i < types.length; i++) types[i]._reset();
    ...
    _registerProfileType: function(profileType) {
        ...
        profileType.addEventListener(
            WebInspector.ProfileType.Events.ViewUpdated,
            this._updateProfileTypeSpecificUI,
            this
        );
        profileType.addEventListener(
            WebInspector.ProfileType.Events.AddProfileHeader,
            onAddProfileHeader,
            this
        );
        profileType.addEventListener(
            WebInspector.ProfileType.Events.RemoveProfileHeader,
            onRemoveProfileHeader,
            this
        );
        var profiles = profileType.getProfiles();
        for (var i = 0; i < profiles.length; i++) this._addProfileHeader(profiles[i]);
    },
    ...
    _addProfileHeader: function(profile) {
        ...
        if (!this.visibleView || this.visibleView === this._launcherView) this.showProfile(profile);
    },
    _removeProfileHeader: function(profile) {
        ...
        if (i !== -1) this._profileToView.splice(i, 1);
        var profileType = profile.profileType();
        var typeId = profileType.id;
    ...
    showProfile: function(profile) {
        if (
            !profile ||
            (profile.profileType().profileBeingRecorded() === profile &&
                !profile.profileType().hasTemporaryView())
        )
            return null;
        var view = this._viewForProfile(profile);
        if (view === this.visibleView) return view;
        this.closeVisibleView();
    ...
    _viewForProfile: function(profile) {
        ...
        if (index !== -1) return this._profileToView[index].view;
    ...
    _indexOfViewForProfile: function(profile) {
        for (var i = 0; i < this._profileToView.length; i++) {
            if (this._profileToView[i].profile === profile) return i;
        }
        return -1;
    },
    ...
    jumpToNextSearchResult: function() {
        if (!this._searchResultsView) return;
        if (this._searchResultsView !== this.visibleView) return;
        this._searchResultsView.jumpToNextSearchResult();
        this._searchableView.updateCurrentMatchIndex(
            this._searchResultsView.currentSearchResultIndex()
        );
    },
    ...
    searchCanceled: function() {
        if (this._searchResultsView) {
            if (this._searchResultsView.searchCanceled) this._searchResultsView.searchCanceled();
    ...
    appendApplicableItems: function(event, contextMenu, target) {
        ...
        if (WebInspector.inspectorView.currentPanel() !== this) return;
        var object = target;
        var objectId = object.objectId;
        if (!objectId) return;
        var heapProfiles = WebInspector.ProfileTypeRegistry.instance.heapSnapshotProfileType.getProfiles();
        if (!heapProfiles.length) return;

        function revealInView(viewName) {
            HeapProfilerAgent.getHeapObjectId(
                objectId,
                didReceiveHeapObjectId.bind(this, viewName)
            );
        }

        function didReceiveHeapObjectId(viewName, error, result) {
            if (WebInspector.inspectorView.currentPanel() !== this) return;
            if (!error) this.showObject(result, viewName);
        }
        if (WebInspector.settings.showAdvancedHeapSnapshotProperties.get())
            contextMenu.appendItem(
                WebInspector.UIString(
                    WebInspector.useLowerCaseMenuTitles()
                        ? 'Reveal in Dominators view'
                        : 'Reveal in Dominators View'
                ),
                revealInView.bind(this, 'Dominators')
            );
        contextMenu.appendItem(
            WebInspector.UIString(
                WebInspector.useLowerCaseMenuTitles()
...
WebInspector.ProfileTypeSidebarSection.prototype = {
    addProfileHeader: function(profile) {
        ...
        if (!profile.fromFile() && profileType.profileBeingRecorded() !== profile) {
            ...
            if (groupSize === 2) {
                group.sidebarTreeElement = new WebInspector.ProfileGroupSidebarTreeElement(
                    profile.title
                );
                ...
                if (selected) firstProfileTreeElement.revealAndSelect();
    ...
    _sidebarElementIndex: function(profile) {
        ...
        for (var i = 0; i < elements.length; i++) {
            if (elements[i].profile === profile) return i;
        }
        return -1;
    },
...
WebInspector.ProfileSidebarTreeElement = function(profile, className) {
    ...
    if (profile.canSaveToFile()) this._createSaveLink();
    else
        profile.addEventListener(
            WebInspector.ProfileHeader.Events.ProfileReceived,
            this._onProfileReceived,
            this
        );
};
WebInspector.ProfileSidebarTreeElement.prototype = {
    ...
    _updateStatus: function(event) {
        ...
        if (statusUpdate.subtitle !== null) this.subtitle = statusUpdate.subtitle;
        if (typeof statusUpdate.wait === 'boolean') this.wait = statusUpdate.wait;
        this.refreshTitles();
    },
    dispose: function() {
        this.profile.removeEventListener(
            WebInspector.ProfileHeader.Events.UpdateStatus,
            this._updateStatus,
            this
        );
    ...
    handleContextMenuEvent: function(event, panel) {
        ...
        contextMenu.appendItem(
            WebInspector.UIString('Load\u2026'),
            panel._fileSelectorElement.click.bind(panel._fileSelectorElement)
        );
        if (profile.canSaveToFile())
            contextMenu.appendItem(
                WebInspector.UIString('Save\u2026'),
                profile.saveToFile.bind(profile)
            );
...
WebInspector.ProfileGroupSidebarTreeElement = function(title, subtitle) {
    WebInspector.SidebarTreeElement.call(
        this,
        'profile-group-sidebar-tree-item',
        title,
        subtitle,
        null,
        true
...
WebInspector.ProfileGroupSidebarTreeElement.prototype = {
    onselect: function() {
        if (this.children.length > 0)
            WebInspector.panels.profiles.showProfile(
                this.children[this.children.length - 1].profile
            );
    },
    __proto__: WebInspector.SidebarTreeElement.prototype
};
...
WebInspector.ProfileDataGridNode = function(profileNode, owningTree, hasChildren) {
    ...
    this._deoptReason =
        !profileNode.deoptReason || profileNode.deoptReason === 'no reason'
            ? ''
            : profileNode.deoptReason;
...
WebInspector.ProfileDataGridNode.prototype = {
    ...
    createCell: function(columnIdentifier) {
        ...
        if (columnIdentifier === 'self' && this._searchMatchedSelfColumn)
            ...
            cell.classList.add('highlight');
        if (columnIdentifier !== 'function') return cell;
        if (this._deoptReason) cell.classList.add('not-optimized');
        if (this.profileNode._searchMatchedFunctionColumn) cell.classList.add('highlight');
        if (this.profileNode.scriptId !== '0') {
            var lineNumber = this.profileNode.lineNumber ? this.profileNode.lineNumber - 1 : 0;
            var columnNumber = this.profileNode.columnNumber
                ? this.profileNode.columnNumber - 1
                : 0;
            var location = new WebInspector.DebuggerModel.Location(
                this.profileNode.scriptId,
                lineNumber,
                columnNumber
            );
            var urlElement = this.tree.profileView._linkifier.linkifyRawLocation(
                location,
                'profile-node-file'
            );
            if (!urlElement)
                urlElement = this.tree.profileView._linkifier.linkifyLocation(
                    this.profileNode.url,
                    lineNumber,
                    columnNumber,
                    'profile-node-file'
                );
    ...
    sort: function(comparator, force) {
        ...
        for (
            var gridNodeGroupIndex = 0;
            gridNodeGroupIndex < gridNodeGroups.length;
            ++gridNodeGroupIndex
        ) {
            ...
            for (var index = 0; index < count; ++index) {
                ...
                if (!force && (!gridNode.expanded || gridNode.lastComparator === comparator)) {
                    if (gridNode.children.length) gridNode.shouldRefreshChildren = true;
                    continue;
                }
    ...
    populate: function() {
        ...
        if (currentComparator) this.sort(currentComparator, true);
    },
    _save: function() {
        if (this._savedChildren) return;
    ...
    _merge: function(child, shouldAbsorb) {
        ...
        if (!shouldAbsorb) this.totalTime += child.totalTime;
        var children = this.children.slice();
        this.removeChildren();
        var count = children.length;
        for (var index = 0; index < count; ++index) {
            if (!shouldAbsorb || children[index] !== child) this.appendChild(children[index]);
        }
        ...
        for (var index = 0; index < count; ++index) {
            ...
            if (existingChild) existingChild._merge(orphanedChild, false);
            else this.appendChild(orphanedChild);
        }
    },
...
WebInspector.ProfileDataGridTree.prototype = {
    ...
    restore: function() {
        ...
        var count = children.length;
        for (var index = 0; index < count; ++index) children[index]._restore();
...
WebInspector.ProfileDataGridTree.propertyComparator = function(property, isAscending) {
    ...
    if (!comparator) {
        ...
        WebInspector.ProfileDataGridTree.propertyComparators[isAscending ? 1 : 0][
            property
        ] = comparator;
    }
    return comparator;
...
WebInspector.BottomUpProfileDataGridNode = function(profileNode, owningTree) {
    WebInspector.ProfileDataGridNode.call(
        this,
        profileNode,
        owningTree,
        this._willHaveChildren(profileNode)
    );
...
WebInspector.BottomUpProfileDataGridNode.prototype = {
    ...
    _exclude: function(aCallUID) {
        if (this._remainingNodeInfos) this.populate();
        this._save();
        var children = this.children;
        var index = this.children.length;
        while (index--) children[index]._exclude(aCallUID);
...
WebInspector.BottomUpProfileDataGridTree = function(profileView, rootProfileNode) {
    ...
    this._remainingNodeInfos = [];
    for (
        var profileNodeGroupIndex = 0;
        profileNodeGroupIndex < profileNodeGroups.length;
...
WebInspector.BottomUpProfileDataGridTree.prototype = {
    ...
    exclude: function(profileDataGridNode) {
        ...
        var children = this.children;
        var count = children.length;
        for (var index = 0; index < count; ++index) children[index]._exclude(excludedCallUID);
...
WebInspector.CPUProfileFlameChart = function(dataProvider) {
    ...
    this._mainPane.addEventListener(
        WebInspector.FlameChart.Events.EntrySelected,
        this._onEntrySelected,
        this
    );
    this._overviewPane._overviewGrid.addEventListener(
...
WebInspector.CPUProfileFlameChart.prototype = {
    _onWindowChanged: function(event) {
        this._mainPane.changeWindow(
            this._overviewPane._overviewGrid.windowLeft(),
...
WebInspector.CPUProfileFlameChart.OverviewPane = function(dataProvider) {
    ...
    this._overviewCanvas = this._overviewContainer.createChild(
        'canvas',
...
WebInspector.CPUProfileFlameChart.OverviewPane.prototype = {
    requestWindowTimes: function(windowStartTime, windowEndTime) {
        this._overviewGrid.setWindow(
            windowStartTime / this._dataProvider.totalTime(),
            windowEndTime / this._dataProvider.totalTime()
        );
    },
    _selectRange: function(timeLeft, timeRight) {
        this._overviewGrid.setWindow(
            timeLeft / this._dataProvider.totalTime(),
...
WebInspector.CPUProfileFlameChart.OverviewPane.calculateDrawData = function(
    dataProvider,
    timelineData,
    width
) {
    ...
    for (var entryIndex = 0; entryIndex < length; ++entryIndex) {
        ...
        var finish = Math.floor(
            (entryOffsets[entryIndex] + entryTotalTimes[entryIndex]) * scaleFactor
        );
...
WebInspector.CPUProfileFlameChart.OverviewPane.drawOverviewCanvas = function(
    dataProvider,
    timelineData,
    context,
    width,
    ...
    var drawData = WebInspector.CPUProfileFlameChart.OverviewPane.calculateDrawData(
        dataProvider,
        timelineData,
        canvasWidth
    );
...
WebInspector.CPUProfileView = function(profileHeader) {
    ...
    this.showSelfTimeAsPercent = WebInspector.settings.createSetting(
        'cpuProfilerShowSelfTimeAsPercent',
        true
    );
    this.showTotalTimeAsPercent = WebInspector.settings.createSetting(
        'cpuProfilerShowTotalTimeAsPercent',
        true
    );
    this.showAverageTimeAsPercent = WebInspector.settings.createSetting(
        'cpuProfilerShowAverageTimeAsPercent',
        true
    );
    this._viewType = WebInspector.settings.createSetting(
        'cpuProfilerView',
        WebInspector.CPUProfileView._TypeFlame
    );
    var columns = [];
    columns.push({
        id: 'self',
    ...
    this.dataGrid.addEventListener(
        WebInspector.DataGrid.Events.SortingChanged,
        this._sortProfile,
        this
    );
    ...
    var options = {};
    options[WebInspector.CPUProfileView._TypeFlame] = this.viewSelectComboBox.createOption(
        WebInspector.UIString('Chart'),
        '',
        WebInspector.CPUProfileView._TypeFlame
    );
    options[WebInspector.CPUProfileView._TypeHeavy] = this.viewSelectComboBox.createOption(
        WebInspector.UIString('Heavy (Bottom Up)'),
        '',
        WebInspector.CPUProfileView._TypeHeavy
    );
    options[WebInspector.CPUProfileView._TypeTree] = this.viewSelectComboBox.createOption(
        WebInspector.UIString('Tree (Top Down)'),
        '',
        WebInspector.CPUProfileView._TypeTree
    );
    ...
    this.focusButton = new WebInspector.StatusBarButton(
        WebInspector.UIString('Focus selected function.'),
    ...
    this.excludeButton = new WebInspector.StatusBarButton(
        WebInspector.UIString('Exclude selected function.'),
    ...
    this.resetButton = new WebInspector.StatusBarButton(
        WebInspector.UIString('Restore all functions.'),
    ...
    if (this.profile._profile) this._processProfileData(this.profile._profile);
    else this._processProfileData(this.profile.protocolProfile());
};
...
WebInspector.CPUProfileView.prototype = {
    ...
    _processProfileData: function(profile) {
        ...
        if (this.samples) this._buildIdToNodeMap();
        this._changeView();
    ...
    refresh: function() {
        var selectedProfileNode = this.dataGrid.selectedNode
            ? this.dataGrid.selectedNode.profileNode
            : null;
    ...
    performSearch: function(query, finishedCallback) {
        ...
        if (greaterThan || lessThan || equalTo) {
            if (equalTo && (greaterThan || lessThan)) queryNumber = parseFloat(query.substring(2));
            else queryNumber = parseFloat(query.substring(1));
        }
        ...
        function matchesQuery(profileDataGridNode) {
            ...
            if (
                profileDataGridNode.functionName.match(matcher) ||
                (profileDataGridNode.url && profileDataGridNode.url.match(matcher))
            )
                profileDataGridNode._searchMatchedFunctionColumn = true;
            if (
                profileDataGridNode._searchMatchedSelfColumn ||
    ...
    showingLastSearchResult: function() {
        return (
            this._searchResults && this._currentSearchResultIndex === this._searchResults.length - 1
        );
    },
    ...
    _changeView: function() {
        ...
        if (this._flameChart) this._flameChart.detach();
        this.dataGrid.show(this.element);
        if (!this.currentQuery || !this._searchFinishedCallback || !this._searchResults) return;
        this._searchFinishedCallback(this, -this._searchResults.length);
        this.performSearch(this.currentQuery, this._searchFinishedCallback);
    },
    _percentClicked: function(event) {
        var currentState =
            this.showSelfTimeAsPercent.get() &&
            this.showTotalTimeAsPercent.get() &&
            this.showAverageTimeAsPercent.get();
    ...
    _updatePercentButton: function() {
        if (
            ...
            this.percentButton.title = WebInspector.UIString(
    ...
    _mouseDownInDataGrid: function(event) {
        ...
        if (
            !cell ||
            (!cell.classList.contains('total-column') &&
                !cell.classList.contains('self-column') &&
    ...
    _assignParentsInProfile: function() {
        if (!this.profileHead) {
            console.error(
                'Unexpected entry into _assignParentsInProfile when we have no profileHead'
            );
            return;
        }
        ...
        while (nodesToTraverse.length) {
            ...
            for (var i = 0; i < length; ++i) {
                ...
                child.head = head;
                child.parent = parent;
                if (child.children.length) nodesToTraverse.push(child);
            }
        }
    },
...
WebInspector.CPUProfileType = function() {
    WebInspector.ProfileType.call(
        this,
        WebInspector.CPUProfileType.TypeId,
        WebInspector.UIString('Collect JavaScript CPU Profile')
    );
...
WebInspector.CPUProfileType.prototype = {
    ...
    get buttonTooltip() {
        return this._recording
            ? WebInspector.UIString('Stop CPU profiling.')
            : WebInspector.UIString('Start CPU profiling.');
    },
    buttonClicked: function() {
        if (this._recording) {
    ...
    consoleProfileStarted: function(id, scriptLocation, title) {
        ...
        if (!resolvedTitle) {
            resolvedTitle = WebInspector.UIString(
                'Profile %s',
                this._nextAnonymousConsoleProfileNumber++
            );
            this._anonymousConsoleProfileIdToTitle[id] = resolvedTitle;
        }
        this._addMessageToConsole(
            WebInspector.ConsoleMessage.MessageType.Profile,
            scriptLocation,
    ...
    consoleProfileFinished: function(protocolId, scriptLocation, cpuProfile, title) {
        ...
        this.addProfile(profile);
        this._addMessageToConsole(
            WebInspector.ConsoleMessage.MessageType.ProfileEnd,
            scriptLocation,
    ...
    startRecordingProfile: function() {
        if (this._profileBeingRecorded) return;
...
WebInspector.CPUProfileHeader = function(type, title) {
    WebInspector.ProfileHeader.call(
        this,
        type,
        title || WebInspector.UIString('Profile %d', type._nextProfileUid)
    );
    this._tempFile = null;
};
WebInspector.CPUProfileHeader.prototype = {
    onTransferStarted: function() {
        ...
        this.updateStatus(
            WebInspector.UIString(
                'Loading\u2026 %s',
                Number.bytesToString(this._jsonifiedProfile.length)
            ),
            true
        );
    },
    ...
    onError: function(reader, e) {
        ...
        switch (e.target.error.code) {
            ...
            case e.target.error.ABORT_ERR:
                return;
            default:
                subtitle = WebInspector.UIString(
                    "'%s' error %d",
                    reader.fileName(),
                    e.target.error.code
                );
    ...
    saveToFile: function() {
        ...
        function onOpenForSave(accepted) {
            ...
            if (this._failedToCreateTempFile) {
                WebInspector.console.log(
                    'Failed to open temp file with heap snapshot',
        ...
        this._fileName =
            this._fileName ||
            'CPU-' + new Date().toISO8601Compact() + this._profileType.fileExtension();
    ...
    _writeToTempFile: function(tempFile, serializedData) {
        ...
        function didWriteToTempFile(success) {
            if (!success) this._failedToCreateTempFile = true;
...
WebInspector.CPUFlameChartDataProvider.prototype = {
    ...
    _calculateTimelineData: function() {
        if (!this._cpuProfileView.profileHead) return null;
        ...
        for (var sampleIndex = 0; sampleIndex < samplesCount; sampleIndex++) {
            ...
            if (depth < openIntervals.length) openIntervals.length = depth;
            if (!node) {
                entries[intervalIndex].selfTime += samplingInterval;
                continue;
            }
            ...
            while (node) {
                entries.push(
                    new ChartEntry(depth, samplingInterval, sampleIndex * samplingInterval, node)
                );
                openIntervals.push({
                    node: node,
                    index: index
    ...
    _millisecondsToString: function(ms) {
        if (ms === 0) return '0';
        if (ms < 1000) return WebInspector.UIString('%.1f\u2009ms', ms);
    ...
    prepareHighlightedEntryInfo: function(entryIndex) {
        ...
        pushEntryInfoRow(
            WebInspector.UIString('Aggregated self time'),
            Number.secondsToString(node.selfTime / 1000, true)
        );
        pushEntryInfoRow(
            WebInspector.UIString('Aggregated total time'),
            Number.secondsToString(node.totalTime / 1000, true)
        );
...
WebInspector.HeapSnapshotCommon.NodeFilter.prototype = {
    equals: function(o) {
        return (
            this.minNodeId === o.minNodeId &&
            this.maxNodeId === o.maxNodeId &&
...
WebInspector.HeapSnapshotWorkerProxy.prototype = {
    ...
    callMethod: function(callback, objectId, methodName) {
        ...
        if (callback) this._callbacks[callId] = callback;
        this._postMessage({
            callId: callId,
            disposition: 'method',
            objectId: objectId,
    ...
    _checkLongRunningCalls: function() {
        for (var callId in this._previousCallbacks)
            if (!(callId in this._callbacks)) delete this._previousCallbacks[callId];
        ...
        for (callId in this._callbacks) this._previousCallbacks[callId] = true;
    },
    _messageReceived: function(event) {
        var data = event.data;
        if (data.eventName) {
            if (this._eventHandler) this._eventHandler(data.eventName, data.data);
            return;
        }
        if (data.error) {
            if (data.errorMethodName)
                WebInspector.console.log(
                    WebInspector.UIString(
                        "An error happened when a call for method '%s' was requested",
                        data.errorMethodName
                    )
                );
        ...
        if (!this._callbacks[data.callId]) return;
...
WebInspector.HeapSnapshotSortableDataGrid = function(columns) {
    ...
    this.addEventListener(
        WebInspector.HeapSnapshotSortableDataGrid.Events.SortingComplete,
        this._sortingComplete,
        this
    );
...
WebInspector.HeapSnapshotSortableDataGrid.prototype = {
    ...
    _disposeAllNodes: function() {
        ...
        for (var i = 0, l = children.length; i < l; ++i) children[i].dispose();
    },
    wasShown: function() {
        if (this._populatedAndSorted)
            this.dispatchEventToListeners(
                WebInspector.HeapSnapshotSortableDataGrid.Events.ContentShown,
                this
            );
    },
    ...
    populateContextMenu: function(contextMenu, event) {
        ...
        if (node && node.showRetainingEdges) {
            ...
            node instanceof WebInspector.HeapSnapshotInstanceNode ||
            node instanceof WebInspector.HeapSnapshotObjectNode
        ) {
            contextMenu.appendItem(
                WebInspector.UIString(
                    WebInspector.useLowerCaseMenuTitles()
    ...
    nodeWasDetached: function(node) {
        if (this._highlightedNode === node) this._clearCurrentHighlight();
    },
...
WebInspector.HeapSnapshotViewportDataGrid.prototype = {
    ...
    updateVisibleNodes: function(force, pathToReveal) {
        ...
        if (!pathToReveal) {
            if (
                !force &&
                scrollTop >= this._topPaddingHeight &&
                scrollBottom >= this._bottomPaddingHeight
            )
                return;
        ...
        if (selectedNode) {
            if (selectedNode.parent) selectedNode.select(true);
            else this.selectedNode = selectedNode;
        }
    },
    _addVisibleNodes: function(parentNode, topBound, bottomBound, pathToReveal) {
        if (!parentNode.expanded) return 0;
        var nodeToReveal = pathToReveal ? pathToReveal[0] : null;
        var restPathToReveal =
            pathToReveal && pathToReveal.length > 1 ? pathToReveal.slice(1) : null;
        var children = this.allChildren(parentNode);
        var topPadding = 0;
        for (var i = 0; i < children.length; ++i) {
            var child = children[i];
            if (child.filteredOut && child.filteredOut()) continue;
            var newTop = topPadding + this._nodeHeight(child);
            if (nodeToReveal === child || (!nodeToReveal && newTop > topBound)) break;
            topPadding = newTop;
        }
        ...
        for (; i < children.length && (nodeToReveal || position < bottomBound); ++i) {
            ...
            position += this._addVisibleNodes(
                child,
                topBound - position,
                bottomBound - position,
                restPathToReveal
            );
            if (nodeToReveal === child) break;
        }
        var bottomPadding = 0;
    ...
    _onScroll: function(event) {
        ...
        if (this._nodeToHighlightAfterScroll) {
            WebInspector.HeapSnapshotSortableDataGrid.prototype.highlightNode.call(
                this,
                this._nodeToHighlightAfterScroll
            );
            this._nodeToHighlightAfterScroll = null;
        }
    },
...
WebInspector.HeapSnapshotContainmentDataGrid.prototype = {
    ...
    sortingChanged: function() {
        ...
        if (rootNode.hasChildren) rootNode.sort();
    },
    __proto__: WebInspector.HeapSnapshotSortableDataGrid.prototype
};
...
WebInspector.HeapSnapshotRetainmentDataGrid.prototype = {
    ...
    setDataSource: function(snapshot, nodeIndex) {
        WebInspector.HeapSnapshotContainmentDataGrid.prototype.setDataSource.call(
            this,
            snapshot,
            nodeIndex
        );
        ...
        function populateComplete() {
            this.removeEventListener(
                WebInspector.HeapSnapshotGridNode.Events.PopulateComplete,
                populateComplete,
                this
            );
            this.expand();
            ...
            dataGrid.dispatchEventToListeners(
...
WebInspector.HeapSnapshotConstructorsDataGrid.prototype = {
    ...
    setDataSource: function(snapshot) {
        this.snapshot = snapshot;
        if (this._profileIndex === -1) this._populateChildren();
    ...
    setSelectionRange: function(minNodeId, maxNodeId) {
        this._populateChildren(
            new WebInspector.HeapSnapshotCommon.NodeFilter(minNodeId, maxNodeId)
        );
    },
    ...
    _aggregatesReceived: function(nodeFilter, aggregates) {
        ...
        if (this._nextRequestedFilter) {
            this.snapshot.aggregatesWithFilter(
                this._nextRequestedFilter,
                this._aggregatesReceived.bind(this, this._nextRequestedFilter)
            );
        ...
        for (var constructor in aggregates)
            this.appendNode(
                this.rootNode(),
                new WebInspector.HeapSnapshotConstructorNode(
                    this,
                    constructor,
    ...
    _populateChildren: function(nodeFilter) {
        ...
        this.snapshot.aggregatesWithFilter(
            nodeFilter,
            this._aggregatesReceived.bind(this, nodeFilter)
        );
    },
    ...
    _populateChildren: function() {
        function aggregatesForDiffReceived(aggregatesForDiff) {
            this.snapshot.calculateSnapshotDiff(
                this.baseSnapshot.uid,
                aggregatesForDiff,
                didCalculateSnapshotDiff.bind(this)
            );
            ...
            function didCalculateSnapshotDiff(diffByClassName) {
                for (var className in diffByClassName) {
                    ...
                    this.appendNode(
                        this.rootNode(),
                        new WebInspector.HeapSnapshotDiffNode(this, className, diff)
                    );
                }
                this.sortingChanged();
            }
...
WebInspector.HeapSnapshotDominatorsDataGrid.prototype = {
    ...
    highlightObjectByHeapSnapshotId: function(id, callback) {
        ...
        function didGetDominators(dominatorIds) {
            if (!dominatorIds) {
                WebInspector.console.log(
                    WebInspector.UIString('Cannot find corresponding heap snapshot node')
                );
                callback(false);
                return;
            }
        ...
        function expandNextDominator(dominatorIds, dominatorNode) {
            ...
            dominatorNode.retrieveChildBySnapshotObjectId(
                snapshotObjectId,
                expandNextDominator.bind(this, dominatorIds)
            );
        }
...
WebInspector.HeapSnapshotGridNode.prototype = {
    ...
    _provider: function() {
        if (!this._providerObject) this._providerObject = this.createProvider();
    ...
    dispose: function() {
        if (this._providerObject) this._providerObject.dispose();
    ...
    _populateChildren: function(fromPosition, toPosition, afterPopulate) {
        ...
        function serializeNextChunk() {
            if (firstNotSerializedPosition >= toPosition) return;
            var end = Math.min(
                firstNotSerializedPosition + this._dataGrid.defaultPopulateCount(),
                toPosition
            );
            this._provider().serializeItemsRange(
                firstNotSerializedPosition,
                end,
        ...
        function insertShowMoreButton(from, to, insertionIndex) {
            var button = new WebInspector.ShowMoreDataGridNode(
                this._populateChildren.bind(this),
                from,
                to,
                this._dataGrid.defaultPopulateCount()
            );
        ...
        function childrenRetrieved(itemsRange) {
            ...
            if (!this._retrievedChildrenRanges.length) {
                ...
                while (rangeIndex < this._retrievedChildrenRanges.length) {
                    ...
                    if (range.to < itemsRange.totalLength) insertionIndex += 1;
                    ++rangeIndex;
                }
                if (!found || itemsRange.startPosition < range.from) {
                    ...
                    if (!found) rangeIndex = this._retrievedChildrenRanges.length;
                ...
                while (range.to < itemsRange.endPosition) {
                    ...
                    if (nextRange && newEndOfRange === nextRange.from) {
                        ...
                        if (newEndOfRange === itemsRange.totalLength)
                            ...
                            this.allChildren()[insertionIndex].setStartPosition(
                                itemsRange.endPosition
                            );
                    }
                }
...
WebInspector.HeapSnapshotGenericObjectNode = function(dataGrid, node) {
    ...
    if (this._type === 'string') this._reachableFromWindow = true;
    ...
    } else if (node.canBeQueried) this._reachableFromWindow = true;
...
WebInspector.HeapSnapshotGenericObjectNode.prototype = {
    ...
    createCell: function(columnIdentifier) {
        var cell =
            columnIdentifier !== 'object'
                ? this._createValueCell(columnIdentifier)
    ...
    _createObjectCell: function() {
        ...
        if (this._reachableFromWindow) valueStyle += ' highlight';
        if (value === 'Object') value = '';
        if (this.detachedDOMTreeNode) valueStyle += ' detached-dom-tree-node';
    ...
    queryObjectContent: function(callback, objectGroupName) {
        ...
        if (this._type === 'string')
            ...
            HeapProfilerAgent.getObjectByHeapObjectId(
    ...
    shortenWindowURL: function(fullName, hasObjectId) {
        ...
        if (startPos !== -1 && endPos !== -1) {
            ...
            if (url.length > 40) url = url.trimMiddle(40);
...
WebInspector.HeapSnapshotObjectNode = function(dataGrid, snapshot, edge, parentGridNode) {
    ...
    if (!this._cycledWithAncestorGridNode) this.updateHasChildren();
    var data = this.data;
    data['count'] = '';
    data['addedCount'] = '';
    data['removedCount'] = '';
...
WebInspector.HeapSnapshotObjectNode.prototype = {
    ...
    createProvider: function() {
        ...
        else return this._snapshot.createEdgesProvider(this.snapshotNodeIndex);
    },
    _findAncestorWithSameSnapshotNodeId: function() {
        var ancestor = this._parentGridNode;
        while (ancestor) {
            if (ancestor.snapshotNodeId === this.snapshotNodeId) return ancestor;
    ...
    _prefixObjectCell: function(div) {
        ...
        if (this._cycledWithAncestorGridNode) div.className += ' cycled-ancessor-node';
...
WebInspector.HeapSnapshotInstanceNode.prototype = {
    ...
    _createChildNode: function(item) {
        return new WebInspector.HeapSnapshotObjectNode(
            this._dataGrid,
...
WebInspector.HeapSnapshotConstructorNode.prototype = {
        ...
        function didGetNodePosition(nodePosition) {
            if (nodePosition === -1) {
                ...
                this._populateChildren(
                    nodePosition,
                    null,
                    didPopulateChildren.bind(this, nodePosition)
                );
            }
        }
    ...
    createCell: function(columnIdentifier) {
        var cell =
            columnIdentifier !== 'object'
                ? this._createValueCell(columnIdentifier)
                : WebInspector.HeapSnapshotGridNode.prototype.createCell.call(
                      this,
                      columnIdentifier
                  );
        if (this._searchMatched) cell.classList.add('highlight');
...
WebInspector.HeapSnapshotDiffNodesProvider = function(
    addedNodesProvider,
...
WebInspector.HeapSnapshotDiffNodesProvider.prototype = {
    ...
    serializeItemsRange: function(beginPosition, endPosition, callback) {
        ...
        function didReceiveAddedItems(itemsRange) {
            var items = itemsRange.items;
            for (var i = 0; i < items.length; i++) items[i].isAddedNotRemoved = true;
            if (itemsRange.endPosition < endPosition)
                return this._deletedNodesProvider.serializeItemsRange(
                    0,
                    endPosition - itemsRange.endPosition,
                    didReceiveDeletedItems.bind(this, itemsRange)
                );
        ...
        if (beginPosition < this._addedCount) {
            this._addedNodesProvider.serializeItemsRange(
                beginPosition,
                endPosition,
                didReceiveAddedItems.bind(this)
            );
        } else {
...
WebInspector.HeapSnapshotDiffNode = function(dataGrid, className, diffForClass) {
    ...
    this.data = {
        ...
        countDelta:
            this._signForDelta(this._countDelta) +
            Number.withThousandsSeparator(Math.abs(this._countDelta)),
        addedSize: Number.withThousandsSeparator(this._addedSize),
        removedSize: Number.withThousandsSeparator(this._removedSize),
        sizeDelta:
            this._signForDelta(this._sizeDelta) +
    ...
    populate: function() {
        ...
        function didReceiveCallers(callers) {
            ...
            for (var i = 0; i < callersChain.length; i++) {
                ...
                if (this.expanded) parentNode.expand();
            }
            ...
            for (var i = 0; i < callersBranch.length; i++)
                parentNode.appendChild(
                    new WebInspector.AllocationGridNode(this._dataGrid, callersBranch[i])
                );
        }
    },
    expand: function() {
        WebInspector.DataGridNode.prototype.expand.call(this);
        if (this.children.length === 1) this.children[0].expand();
    },
    createCell: function(columnIdentifier) {
        var cell = WebInspector.DataGridNode.prototype.createCell.call(this, columnIdentifier);
        if (columnIdentifier !== 'name') return cell;
        var functionInfo = this.data;
        if (functionInfo.scriptName) {
            var urlElement = this._dataGrid._linkifier.linkifyLocation(
                functionInfo.scriptName,
                functionInfo.line - 1,
                functionInfo.column - 1,
                'profile-node-file'
            );
            urlElement.style.maxWidth = '75%';
            cell.insertBefore(urlElement, cell.firstChild);
...
WebInspector.HeapSnapshotView = function(profile) {
    ...
    profile
        .profileType()
        .addEventListener(
            WebInspector.HeapSnapshotProfileType.SnapshotReceived,
    ...
    if (profile._profileType.id === WebInspector.TrackingHeapSnapshotProfileType.TypeId) {
        ...
        this._trackingOverviewGrid.addEventListener(
            WebInspector.HeapTrackingOverviewGrid.IdsRangeChanged,
            this._onIdsRangeChanged.bind(this)
        );
    }
    this._splitView = new WebInspector.SplitView(
        false,
        true,
    ...
    this._containmentDataGrid.addEventListener(
        WebInspector.DataGrid.Events.SelectedNode,
    ...
    this._diffDataGrid.addEventListener(
        WebInspector.DataGrid.Events.SelectedNode,
        this._selectionChanged,
        this
    );
    ...
    this._dominatorDataGrid.addEventListener(
        WebInspector.DataGrid.Events.SelectedNode,
        this._selectionChanged,
        this
    );
    if (
        WebInspector.experimentsSettings.allocationProfiler.isEnabled() &&
        profile.profileType() ===
            WebInspector.ProfileTypeRegistry.instance.trackingHeapSnapshotProfileType
    ) {
    ...
    this._perspectiveSelect = new WebInspector.StatusBarComboBox(
        this._onSelectedPerspectiveChanged.bind(this)
    );
    ...
    this._popoverHelper = new WebInspector.ObjectPopoverHelper(
        this.element,
        this._getHoverAnchor.bind(this),
    ...
    this._dataGrid.addEventListener(
        WebInspector.HeapSnapshotSortableDataGrid.Events.ResetFilter,
        this._onResetClassNameFilter,
        this
    );
    this._refreshView();
};
...
WebInspector.HeapSnapshotView.Perspective.prototype = {
    ...
    deactivate: function(heapSnapshotView) {
        ...
        if (heapSnapshotView._trackingOverviewGrid) heapSnapshotView._trackingOverviewGrid.detach();
        if (heapSnapshotView._allocationView) heapSnapshotView._allocationView.detach();
...
WebInspector.HeapSnapshotView.AllocationPerspective = function() {
    ...
    this._allocationSplitView = new WebInspector.SplitView(
        false,
        true,
        'heapSnapshotAllocationSplitViewState',
        200,
        200
    );
};
WebInspector.HeapSnapshotView.AllocationPerspective.prototype = {
    activate: function(heapSnapshotView) {
        ...
        if (selectedNode)
            heapSnapshotView._constructorsDataGrid.setAllocationNodeId(
                selectedNode.allocationNodeId()
            );
    },
    deactivate: function(heapSnapshotView) {
...
WebInspector.HeapSnapshotView.prototype = {
    _refreshView: function() {
        ...
        function profileCallback(heapSnapshotProxy) {
            ...
            if (this._trackingOverviewGrid) this._trackingOverviewGrid._updateGrid();
        }
    },
    _gotStatistics: function(statistics) {
        ...
        this._statisticsView.addRecord(
            statistics.strings,
            WebInspector.UIString('Strings'),
            '#5e5'
        );
        this._statisticsView.addRecord(
            statistics.jsArrays,
            WebInspector.UIString('JS Arrays'),
            '#7af'
        );
        this._statisticsView.addRecord(
            statistics.native,
            WebInspector.UIString('Typed Arrays'),
            '#fc5'
        );
    ...
    _onIdsRangeChanged: function(event) {
        ...
        this._selectedSizeText.setText(
            WebInspector.UIString('Selected size: %s', Number.bytesToString(event.data.size))
        );
    ...
    wasShown: function() {
        ...
        function profileCallback() {
            ...
            if (this._baseProfile) this._baseProfile.load(function() {});
        }
    },
    willHide: function() {
        this._currentSearchResultIndex = -1;
        this._popoverHelper.hidePopover();
        if (this.helpPopover && this.helpPopover.isShowing()) this.helpPopover.hide();
    },
    searchCanceled: function() {
        if (this._searchResults) {
            for (var i = 0; i < this._searchResults.length; ++i) {
    ...
    performSearch: function(query, finishedCallback) {
        ...
        query = query.trim();
        if (!query) return;
        if (!this._currentPerspective.supportsSearch()) return;
        ...
        if (query.charAt(0) === '@') {
            ...
            if (!isNaN(snapshotNodeId))
                this._dataGrid.highlightObjectByHeapSnapshotId(
                    String(snapshotNodeId),
                    didHighlight.bind(this)
                );
            else finishedCallback(this, 0);
            return;
        }
        this._searchFinishedCallback = finishedCallback;
    ...
    _changeBase: function() {
        if (this._baseProfile === this._profiles()[this._baseSelect.selectedIndex()]) return;
        this._baseProfile = this._profiles()[this._baseSelect.selectedIndex()];
        var dataGrid = this._dataGrid;
        if (dataGrid.snapshot) this._baseProfile.load(dataGrid.setBaseDataSource.bind(dataGrid));
        if (!this.currentQuery || !this._searchFinishedCallback || !this._searchResults) return;
        this._searchFinishedCallback(this, -this._searchResults.length);
        this.performSearch(this.currentQuery, this._searchFinishedCallback);
    },
    ...
    populateContextMenu: function(contextMenu, event) {
        if (this._dataGrid) this._dataGrid.populateContextMenu(contextMenu, event);
    },
    ...
    _inspectedObjectChanged: function(event) {
        ...
        if (
            ...
            selectedNode instanceof WebInspector.HeapSnapshotGenericObjectNode
        )
    ...
    _setRetainmentDataGridSource: function(nodeItem) {
        ...
        if (dataSource)
            this._retainmentDataGrid.setDataSource(
                dataSource.snapshot,
                dataSource.snapshotNodeIndex
            );
        else this._retainmentDataGrid.reset();
    },
    _changePerspectiveAndWait: function(perspectiveTitle, callback) {
        ...
        function dataGridContentShown(event) {
            ...
            if (dataGrid === this._dataGrid) callback();
        }
        this._perspectives[perspectiveIndex]
            .masterGrid(this)
            .addEventListener(
                WebInspector.HeapSnapshotSortableDataGrid.Events.ContentShown,
                dataGridContentShown,
    ...
    _updateDataSourceAndView: function() {
        var dataGrid = this._dataGrid;
        if (!dataGrid || dataGrid.snapshot) return;
    ...
    _changePerspective: function(selectedIndex) {
        if (selectedIndex === this._currentPerspectiveIndex) return;
        if (this._dataGrid)
            this._dataGrid.removeEventListener(
                WebInspector.HeapSnapshotSortableDataGrid.Events.ResetFilter,
                this._onResetClassNameFilter,
                this
            );
    ...
    highlightLiveObject: function(perspectiveName, snapshotObjectId) {
        ...
        function didHighlightObject(found) {
            if (!found)
                WebInspector.console.log(
                    'Cannot find corresponding heap snapshot node',
                    WebInspector.ConsoleMessage.MessageLevel.Error,
                    true
                );
        }
    ...
    _resolveObjectForPopover: function(element, showCallback, objectGroupName) {
        if (this._profile.fromFile()) return;
        element.node.queryObjectContent(showCallback, objectGroupName);
    },
    _updateBaseOptions: function() {
        var list = this._profiles();
        if (this._baseSelect.size() === list.length) return;
    ...
    _updateFilterOptions: function() {
        var list = this._profiles();
        if (this._filterSelect.size() - 1 === list.length) return;
        ...
        for (var i = this._filterSelect.size() - 1, n = list.length; i < n; ++i) {
            var title = list[i].title;
            if (!i) title = WebInspector.UIString('Objects allocated before %s', title);
            else
                title = WebInspector.UIString(
                    'Objects allocated between %s and %s',
                    list[i - 1].title,
                    title
                );
...
WebInspector.HeapSnapshotProfileType = function(id, title) {
    WebInspector.ProfileType.call(
        this,
        id || WebInspector.HeapSnapshotProfileType.TypeId,
        title || WebInspector.UIString('Take Heap Snapshot')
    );
...
WebInspector.HeapSnapshotProfileType.prototype = {
    ...
    _takeHeapSnapshot: function(callback) {
        if (this.profileBeingRecorded()) return;
    ...
    reportHeapSnapshotProgress: function(done, total, finished) {
        ...
        if (finished) profile._prepareToLoad();
    },
    resetProfiles: function() {
        this._reset();
    },
    _snapshotReceived: function(profile) {
        if (this._profileBeingRecorded === profile) this._profileBeingRecorded = null;
        this.dispatchEventToListeners(
            WebInspector.HeapSnapshotProfileType.SnapshotReceived,
            profile
        );
...
WebInspector.TrackingHeapSnapshotProfileType = function() {
    WebInspector.HeapSnapshotProfileType.call(
        this,
        WebInspector.TrackingHeapSnapshotProfileType.TypeId,
        WebInspector.UIString('Record Heap Allocations')
    );
};
...
WebInspector.TrackingHeapSnapshotProfileType.prototype = {
    heapStatsUpdate: function(samples) {
        if (!this._profileSamples) return;
        var index;
        for (var i = 0; i < samples.length; i += 3) {
            ...
            if (!this._profileSamples.max[index]) this._profileSamples.max[index] = size;
        }
    },
    lastSeenObjectId: function(lastSeenObjectId, timestamp) {
        ...
        this.dispatchEventToListeners(
            WebInspector.TrackingHeapSnapshotProfileType.HeapStatsUpdate,
    ...
    get buttonTooltip() {
        return this._recording
            ? WebInspector.UIString('Stop recording heap profile.')
    ...
    _toggleRecording: function() {
        if (this._recording) this._stopRecordingProfile();
        else this._startRecordingProfile();
        return this._recording;
    },
    get treeItemTitle() {
    ...
    resetProfiles: function() {
        ...
        this._profileSamples = null;
        this._lastSeenIndex = -1;
        if (wasRecording) this._addNewProfile();
    },
    profileBeingRecordedRemoved: function() {
...
WebInspector.HeapProfileHeader = function(type, title) {
    WebInspector.ProfileHeader.call(
        this,
        type,
        title || WebInspector.UIString('Snapshot %d', type._nextProfileUid)
    );
...
WebInspector.HeapProfileHeader.prototype = {
    ...
    _finishLoad: function() {
        if (!this._wasDisposed) this._receiver.close(function() {});
    ...
    _didWriteToTempFile: function(tempFile) {
        ...
        this._tempFile = tempFile;
        if (!tempFile) this._failedToCreateTempFile = true;
        if (this._onTempFileReady) {
            this._onTempFileReady();
            this._onTempFileReady = null;
        }
    ...
    _setupWorker: function() {
        ...
        this._workerProxy = new WebInspector.HeapSnapshotWorkerProxy(
            this._handleWorkerEvent.bind(this)
        );
        this._workerProxy.addEventListener('wait', setProfileWait, this);
        this._receiver = this._workerProxy.createLoader(
            this.uid,
            this._snapshotReceived.bind(this)
        );
    },
    _handleWorkerEvent: function(eventName, data) {
        if (WebInspector.HeapSnapshotProgressEvent.Update !== eventName) return;
        var subtitle = data;
        this.updateStatus(subtitle);
    },
    dispose: function() {
        if (this._workerProxy) this._workerProxy.dispose();
        this.removeTempFile();
        this._wasDisposed = true;
    },
    _didCompleteSnapshotTransfer: function() {
        if (!this._snapshotProxy) return;
        this.updateStatus(Number.bytesToString(this._snapshotProxy.totalSize), false);
    },
    transferChunk: function(chunk) {
        if (!this._bufferedWriter)
            this._bufferedWriter = new WebInspector.BufferedTempFileWriter(
                'heap-profiler',
                this.uid
            );
    ...
    saveToFile: function() {
        ...
        this._fileName =
            this._fileName ||
            'Heap-' + new Date().toISO8601Compact() + this._profileType.fileExtension();
...
WebInspector.SaveSnapshotOutputStreamDelegate.prototype = {
    ...
    onError: function(reader, event) {
        WebInspector.console.log(
            'Failed to read heap snapshot from temp file: ' + event.message,
            WebInspector.ConsoleMessage.MessageLevel.Error
        );
...
WebInspector.HeapTrackingOverviewGrid = function(heapProfileHeader) {
    ...
    if (heapProfileHeader.profileType().profileBeingRecorded() === heapProfileHeader) {
        ...
        this._profileType.addEventListener(
            WebInspector.TrackingHeapSnapshotProfileType.TrackingStopped,
    ...
    this._windowRight =
        totalTime && timestamps.length
            ? (timestamps[timestamps.length - 1] - timestamps[0]) / totalTime
            : 1.0;
...
WebInspector.HeapTrackingOverviewGrid.prototype = {
    ...
    _drawOverviewCanvas: function(width, height) {
        ...
        function aggregateAndCall(sizes, callback) {
            ...
            for (var i = 1; i < timestamps.length; ++i) {
                ...
                if (x !== currentX) {
                    if (size) callback(currentX, size);
                    size = 0;
                    currentX = x;
                }
                size += sizes[i];
        ...
        if (gridValue) {
            ...
            context.font =
                '10px ' +
                window.getComputedStyle(this.element, null).getPropertyValue('font-family');
    ...
    _updateGrid: function() {
        ...
        for (var i = 0; i < timestamps.length; ++i) {
            ...
            if (timestamps[i] > timeRight) break;
            maxId = ids[i];
...
WebInspector.HeapTrackingOverviewGrid.SmoothScale.prototype = {
    nextScale: function(target) {
        ...
        if (this._currentScale) {
            ...
            this._currentScale *= Number.constrain(
                scaleChange,
                1 / maxChangePerDelta,
                maxChangePerDelta
            );
        } else this._currentScale = target;
...
WebInspector.HeapSnapshotStatisticsView.prototype = {
    ...
    addRecord: function(value, name, color) {
        if (color) this._pieChart.addSlice(value, color);
        ...
        if (color) swatchDiv.style.backgroundColor = color;
        else swatchDiv.classList.add('heap-snapshot-stats-empty-swatch');
        nameDiv.textContent = name;
        sizeDiv.textContent = WebInspector.UIString(
            '%s KB',
            Number.withThousandsSeparator(Math.round(value / 1024))
        );
    },
    __proto__: WebInspector.VBox.prototype
};
...
WebInspector.ProfileLauncherView.prototype = {
    addProfileType: function(profileType) {
        ...
        if (decorationElement) this._innerContentElement.appendChild(decorationElement);
    ...
    _updateControls: function() {
        if (this._isEnabled) this._controlButton.removeAttribute('disabled');
        else this._controlButton.setAttribute('disabled', '');
...
WebInspector.MultiProfileLauncherView = function(profilesPanel) {
    ...
    WebInspector.settings.selectedProfileType = WebInspector.settings.createSetting(
        'selectedProfileType',
        'CPU'
    );
...
WebInspector.MultiProfileLauncherView.prototype = {
    addProfileType: function(profileType) {
        ...
        optionElement.addEventListener(
            'change',
            this._profileTypeChanged.bind(this, profileType),
            false
        );
        var descriptionElement = labelElement.createChild('p');
        descriptionElement.textContent = profileType.description;
        var decorationElement = profileType.decorationElement();
        if (decorationElement) labelElement.appendChild(decorationElement);
    },
    ...
    _updateControls: function() {
        ...
        for (var i = 0; i < items.length; ++i) {
            if (items[i].type === 'radio') items[i].disabled = this._isProfiling;
        }
    },
...
WebInspector.TopDownProfileDataGridNode.prototype = {
    ...
    _exclude: function(aCallUID) {
        if (this._remainingChildren) this.populate();
        this._save();
        var children = this.children;
        var index = this.children.length;
        while (index--) children[index]._exclude(aCallUID);
...
WebInspector.CanvasProfileView = function(profile) {
    ...
    this._linkifier = new WebInspector.Linkifier();
    this._replayInfoSplitView = new WebInspector.SplitView(
        true,
        true,
        'canvasProfileViewReplaySplitViewState',
        0.34
    );
    this._replayInfoSplitView.show(this.element);
    this._imageSplitView = new WebInspector.SplitView(
        false,
        true,
        'canvasProfileViewSplitViewState',
        300
    );
    ...
    this._createControlButton(
        controlsContainer,
        'canvas-replay-first-step',
        WebInspector.UIString('First call.'),
        this._onReplayFirstStepClick.bind(this)
    );
    this._createControlButton(
        controlsContainer,
        'canvas-replay-prev-step',
        WebInspector.UIString('Previous call.'),
        this._onReplayStepClick.bind(this, false)
    );
    this._createControlButton(
        controlsContainer,
        'canvas-replay-next-step',
        WebInspector.UIString('Next call.'),
        this._onReplayStepClick.bind(this, true)
    );
    this._createControlButton(
        controlsContainer,
        'canvas-replay-prev-draw',
        WebInspector.UIString('Previous drawing call.'),
        this._onReplayDrawingCallClick.bind(this, false)
    );
    this._createControlButton(
        controlsContainer,
        'canvas-replay-next-draw',
        WebInspector.UIString('Next drawing call.'),
        this._onReplayDrawingCallClick.bind(this, true)
    );
    this._createControlButton(
        controlsContainer,
        'canvas-replay-last-step',
        WebInspector.UIString('Last call.'),
        this._onReplayLastStepClick.bind(this)
    );
    this._replayContextSelector = new WebInspector.StatusBarComboBox(
        this._onReplayContextChanged.bind(this)
    );
    this._replayContextSelector.createOption(
        WebInspector.UIString('<screenshot auto>'),
        WebInspector.UIString('Show screenshot of the last replayed resource.'),
        ''
    );
    ...
    this._logGrid.addEventListener(
        WebInspector.DataGrid.Events.SelectedNode,
...
WebInspector.CanvasProfileView.prototype = {
    ...
    _installReplayInfoSidebarWidgets: function(controlsContainer) {
        this._replayInfoResizeWidgetElement = controlsContainer.createChild(
            'div',
            'resizer-widget'
        );
        this._replayInfoSplitView.addEventListener(
        ...
        this._toggleReplayStateSidebarButton = this._replayInfoSplitView.createShowHideSidebarButton(
            'sidebar',
            'canvas-sidebar-show-hide-button'
        );
    ...
    _updateReplayInfoResizeWidget: function() {
        this._replayInfoResizeWidgetElement.classList.toggle(
            'hidden',
            this._replayInfoSplitView.showMode() !== WebInspector.SplitView.ShowMode.Both
        );
    },
    _onMouseClick: function(event) {
        var resourceLinkElement = event.target.enclosingNodeOrSelfWithClass(
    ...
    _onReplayContextChanged: function() {
        ...
        function didReceiveResourceState(resourceState) {
            ...
            if (selectedContextId !== this._replayContextSelector.selectedOption().value) return;
        ...
        this._traceLogPlayer.getResourceState(
            selectedContextId,
            didReceiveResourceState.bind(this)
        );
    },
    _onReplayStepClick: function(forward) {
        ...
        do {
            nextNode = forward
                ? nextNode.traverseNextNode(false)
                : nextNode.traversePreviousNode(false);
    ...
    _onReplayDrawingCallClick: function(forward) {
        ...
        while (nextNode) {
            ...
            if (sibling) {
                nextNode = sibling;
                if (nextNode.hasChildren || nextNode.call.isDrawingCall) break;
            } else {
                nextNode = nextNode.parent;
                if (!forward) break;
            }
        }
        if (!nextNode && forward) this._onReplayLastStepClick();
        else (nextNode || selectedNode).revealAndSelect();
    },
    ...
    _replayTraceLog: function() {
        if (this._pendingReplayTraceLogEvent) return;
        var index = this._selectedCallIndex();
        if (index === -1 || index === this._lastReplayCallIndex) return;
        ...
        function didReplayTraceLog(resourceState, replayTime) {
            ...
            this._enableWaitIcon(false);
            this._debugInfoElement.textContent = WebInspector.UIString(
                'Replay time: %s',
                Number.secondsToString(replayTime / 1000, true)
            );
            this._onReplayContextChanged();
            if (index !== this._selectedCallIndex()) this._replayTraceLog();
        }
        this._enableWaitIcon(true);
    ...
    _requestTraceLog: function(offset) {
        function didReceiveTraceLog(traceLog) {
            ...
            for (var i = 0, n = contexts.length; i < n; ++i) {
                ...
                if (this._replayContexts[contextId]) continue;
                this._replayContexts[contextId] = true;
                this._replayContextSelector.createOption(
                    description,
                    WebInspector.UIString("Show screenshot of this context's canvas."),
                    contextId
                );
            }
            ...
            if (traceLog.alive)
                setTimeout(
                    this._requestTraceLog.bind(this, index),
                    WebInspector.CanvasProfileView.TraceLogPollingInterval
                );
            else this._flattenSingleFrameNode();
    ...
    _peekLastRecursively: function(node) {
        var lastChild;
        while ((lastChild = node.children.peekLast())) node = lastChild;
        return node;
    },
    _appendCallNodes: function(callNodes) {
        var rootNode = this._logGrid.rootNode();
        var frameNode = rootNode.children.peekLast();
        if (frameNode && this._peekLastRecursively(frameNode).call.isFrameEndCall) frameNode = null;
    ...
    _appendCallNodesToFrameNode: function(frameNode, callNodes, fromIndex, toIndex) {
        ...
        function splitDrawCallGroup(drawCallGroup) {
            ...
            while ((splitNode = drawCallGroup.children[splitIndex])) {
                if (splitNode.call.isDrawingCall) break;
                ++splitIndex;
            }
        ...
        } else drawCallGroup = appendDrawCallGroup();
        for (var i = fromIndex; i < toIndex; ++i) {
            var node = callNodes[i];
            drawCallGroup.appendChild(node);
            if (node.call.isDrawingCall) {
                if (groupHasDrawCall) drawCallGroup = splitDrawCallGroup(drawCallGroup);
                else groupHasDrawCall = true;
            }
        }
    },
    _createCallNode: function(index, call) {
        ...
        callViewElement.createChild('span', 'canvas-function-name').textContent =
            call.functionName || 'context.' + call.property;
        if (call.arguments) {
            callViewElement.createTextChild('(');
            for (var i = 0, n = call.arguments.length; i < n; ++i) {
                var argument = call.arguments[i];
                if (i) callViewElement.createTextChild(', ');
                var element = WebInspector.CanvasProfileDataGridHelper.createCallArgumentElement(
                    argument
                );
            ...
            callViewElement.createTextChild(' = ');
            callViewElement.appendChild(
                WebInspector.CanvasProfileDataGridHelper.createCallArgumentElement(call.value)
            );
        }
        if (call.result) {
            callViewElement.createTextChild(' => ');
            callViewElement.appendChild(
                WebInspector.CanvasProfileDataGridHelper.createCallArgumentElement(call.result)
            );
        }
        var node = new WebInspector.DataGridNode(data);
        node.index = index;
    ...
    _popoverAnchor: function(element, event) {
        ...
        if (!argumentElement || argumentElement.__suppressPopover) return null;
    ...
    _resolveObjectForPopover: function(argumentElement, showCallback, objectGroupName) {
        function showObjectPopover(error, result, resourceState) {
            ...
            var diffLeft =
                this._popoverAnchorElement.boxInWindow().x - argumentElement.boxInWindow().x;
            this._popoverAnchorElement.style.left =
                this._popoverAnchorElement.offsetLeft - diffLeft + 'px';
            showCallback(
                WebInspector.RemoteObject.fromPayload(result),
                false,
                this._popoverAnchorElement
            );
        }
        var evalResult = argumentElement.__evalResult;
        if (evalResult) showObjectPopover.call(this, null, evalResult);
        else {
            ...
            if (typeof argumentIndex !== 'number') argumentIndex = -1;
            CanvasAgent.evaluateTraceLogCallArgument(
                this._traceLogId,
                callIndex,
                argumentIndex,
                objectGroupName,
    ...
    _flattenSingleFrameNode: function() {
        var rootNode = this._logGrid.rootNode();
        if (rootNode.children.length !== 1) return;
        var frameNode = rootNode.children[0];
        while (frameNode.children[0]) rootNode.appendChild(frameNode.children[0]);
...
WebInspector.CanvasProfileType = function() {
    WebInspector.ProfileType.call(
        this,
        WebInspector.CanvasProfileType.TypeId,
        WebInspector.UIString('Capture Canvas Frame')
    );
    this._recording = false;
    this._lastProfileHeader = null;
    this._capturingModeSelector = new WebInspector.StatusBarComboBox(
        this._dispatchViewUpdatedEvent.bind(this)
    );
    this._capturingModeSelector.element.title = WebInspector.UIString('Canvas capture mode.');
    this._capturingModeSelector.createOption(
        WebInspector.UIString('Single Frame'),
        WebInspector.UIString('Capture a single canvas frame.'),
        ''
    );
    this._capturingModeSelector.createOption(
        WebInspector.UIString('Consecutive Frames'),
        WebInspector.UIString('Capture consecutive canvas frames.'),
        '1'
    );
    this._frameOptions = {};
    ...
    this._frameSelector.element.title = WebInspector.UIString(
        'Frame containing the canvases to capture.'
    );
    ...
    WebInspector.resourceTreeModel.addEventListener(
        WebInspector.ResourceTreeModel.EventTypes.FrameAdded,
        this._frameAdded,
        this
    );
    WebInspector.resourceTreeModel.addEventListener(
...
WebInspector.CanvasProfileType.prototype = {
    ...
    get buttonTooltip() {
        if (this._isSingleFrameMode()) return WebInspector.UIString('Capture next canvas frame.');
        else
            return this._recording
                ? WebInspector.UIString('Stop capturing canvas frames.')
                : WebInspector.UIString('Start capturing canvas frames.');
    },
    buttonClicked: function() {
        if (!this._canvasAgentEnabled) return false;
        if (this._recording) {
            this._recording = false;
    ...
    _didStartCapturingFrame: function(frameId, error, traceLogId) {
        if (
            error ||
            (this._lastProfileHeader && this._lastProfileHeader.traceLogId() === traceLogId)
        )
            return;
    ...
    removeProfile: function(profile) {
        ...
        if (this._recording && profile === this._lastProfileHeader) this._recording = false;
    },
    _updateDecorationElement: function(forcePageReload) {
        ...
        this._decorationElement.appendChild(
            document.createTextNode(
                this._canvasAgentEnabled
                    ? WebInspector.UIString('Canvas Profiler is enabled.')
                    : WebInspector.UIString('Canvas Profiler is disabled.')
            )
        );
        var button = this._decorationElement.createChild('button');
        button.type = 'button';
        button.textContent = this._canvasAgentEnabled
            ? WebInspector.UIString('Disable')
            : WebInspector.UIString('Enable');
        button.addEventListener(
            'click',
            this._onProfilerEnableButtonClick.bind(this, !this._canvasAgentEnabled),
            false
        );

        function hasUninstrumentedCanvasesCallback(error, result) {
            if (error || result) WebInspector.resourceTreeModel.reloadPage();
        }
        if (forcePageReload) {
    ...
    _onProfilerEnableButtonClick: function(enable) {
        if (this._canvasAgentEnabled === enable) return;
        ...
        if (enable) CanvasAgent.enable(callback.bind(this));
        else CanvasAgent.disable(callback.bind(this));
    },
...
WebInspector.CanvasProfileHeader = function(type, traceLogId, frameId) {
    WebInspector.ProfileHeader.call(
        this,
        type,
        WebInspector.UIString('Trace Log %d', type._nextProfileUid)
    );
    this._traceLogId = traceLogId || '';
    this._frameId = frameId;
    this._alive = true;
    this._traceLogSize = 0;
    this._traceLogPlayer = traceLogId
        ? new WebInspector.CanvasTraceLogPlayerProxy(traceLogId)
        : null;
};
WebInspector.CanvasProfileHeader.prototype = {
    ...
    dispose: function() {
        if (this._traceLogPlayer) this._traceLogPlayer.dispose();
        clearTimeout(this._requestStatusTimer);
        this._alive = false;
    },
    _updateCapturingStatus: function(traceLog) {
        if (!this._traceLogId) return;
        if (traceLog) {
            this._alive = traceLog.alive;
            this._traceLogSize = traceLog.totalAvailableCalls;
        }
        var subtitle = this._alive
            ? WebInspector.UIString('Capturing\u2026 %d calls', this._traceLogSize)
            : WebInspector.UIString('Captured %d calls', this._traceLogSize);
        this.updateStatus(subtitle, this._alive);
        if (this._alive) {
            clearTimeout(this._requestStatusTimer);
            this._requestStatusTimer = setTimeout(
                this._requestCapturingStatus.bind(this),
                WebInspector.CanvasProfileView.TraceLogPollingInterval
            );
        }
    },
...
WebInspector.CanvasProfileDataGridHelper = {
    createCallArgumentElement: function(callArgument) {
        if (callArgument.enumName)
            return WebInspector.CanvasProfileDataGridHelper.createEnumValueElement(
                callArgument.enumName,
                +callArgument.description
            );
        ...
        if (callArgument.type === 'string') {
            ...
            ).textContent = description.trimMiddle(maxStringLength);
            element.createTextChild('"');
            element.__suppressPopover =
                description.length <= maxStringLength && !/[\r\n]/.test(description);
            ...
            if (callArgument.remoteObject)
                element.__evalResult = WebInspector.RemoteObject.fromPayload(
                    callArgument.remoteObject
                );
        }
...
WebInspector.CanvasTraceLogPlayerProxy.prototype = {
    getTraceLog: function(startOffset, maxLength, userCallback) {
        function callback(error, traceLog) {
            ...
            this.dispatchEventToListeners(
                WebInspector.CanvasTraceLogPlayerProxy.Events.CanvasTraceLogReceived,
                traceLog
            );
        }
    ...
    dispose: function() {
        ...
        this.dispatchEventToListeners(
            WebInspector.CanvasTraceLogPlayerProxy.Events.CanvasReplayStateChanged
        );
    },
    getResourceState: function(resourceId, userCallback) {
        ...
        function callback(error, resourceState) {
            ...
            this.dispatchEventToListeners(
                WebInspector.CanvasTraceLogPlayerProxy.Events.CanvasResourceStateReceived,
                resourceState
            );
        }
...
WebInspector.CanvasReplayStateView = function(traceLogPlayer) {
    ...
    this._prevButton = this._createControlButton(
        controlsContainer,
        'canvas-replay-state-prev',
        WebInspector.UIString('Previous resource.'),
        this._onResourceNavigationClick.bind(this, false)
    );
    this._nextButton = this._createControlButton(
        controlsContainer,
        'canvas-replay-state-next',
        WebInspector.UIString('Next resource.'),
        this._onResourceNavigationClick.bind(this, true)
    );
    this._createControlButton(
        controlsContainer,
        'canvas-replay-state-refresh',
        WebInspector.UIString('Refresh.'),
        this._onStateRefreshClick.bind(this)
    );
    this._resourceSelector = new WebInspector.StatusBarComboBox(
        this._onReplayResourceChanged.bind(this)
    );
    this._currentOption = this._resourceSelector.createOption(
        WebInspector.UIString('<auto>'),
        WebInspector.UIString('Show state of the last replayed resource.'),
        ''
    );
    ...
    this._traceLogPlayer.addEventListener(
        WebInspector.CanvasTraceLogPlayerProxy.Events.CanvasReplayStateChanged,
    ...
    this._traceLogPlayer.addEventListener(
        WebInspector.CanvasTraceLogPlayerProxy.Events.CanvasTraceLogReceived,
        this._onCanvasTraceLogReceived,
        this
    );
    this._traceLogPlayer.addEventListener(
        WebInspector.CanvasTraceLogPlayerProxy.Events.CanvasResourceStateReceived,
        this._onCanvasResourceStateReceived,
        this
    );
...
WebInspector.CanvasReplayStateView.prototype = {
    selectResource: function(resourceId) {
        if (resourceId === this._resourceSelector.selectedOption().value) return;
    ...
    _updateCurrentOption: function() {
        ...
        if (this._currentOption === selectedOption) return;
        if (!this._isNavigationButton) {
            this._prevOptionsStack.push(this._currentOption);
            this._nextOptionsStack = [];
            if (this._prevOptionsStack.length > maxStackSize) this._prevOptionsStack.shift();
            this._updateButtonsEnabledState();
        }
    ...
    _collectResourcesFromResourceState: function(resourceState) {
        var collectedResources = [];
        this._collectResourceFromResourceStateDescriptors(
            resourceState.descriptors,
            collectedResources
        );
        this._addCollectedResourcesToSelector(collectedResources);
    },
    ...
    _collectResourceFromCallArgument: function(argument, output) {
        ...
        if (!resourceId || this._resourceIdToDescription[resourceId]) return;
    ...
    _addCollectedResourcesToSelector: function(collectedResources) {
        if (!collectedResources.length) return;
        ...
        for (var i = 0, n = collectedResources.length; i < n; ++i) {
            ...
            while (
                currentOption &&
                String.naturalOrderComparator(currentOption.text, argument.description) < 0
            )
                currentOption = currentOption.nextSibling;
            var option = this._resourceSelector.createOption(
                argument.description,
                WebInspector.UIString('Show state of this resource.'),
                argument.resourceId
            );
            if (currentOption) selectElement.insertBefore(option, currentOption);
        }
    },
    _onReplayResourceChanged: function() {
        ...
        function didReceiveResourceState(resourceState) {
            if (selectedResourceId !== this._resourceSelector.selectedOption().value) return;
    ...
    _showResourceState: function(resourceState) {
        ...
        function comparator(d1, d2) {
            ...
            if (hasChildren1 !== hasChildren2) return hasChildren1 ? 1 : -1;
        ...
        function appendResourceStateDescriptors(descriptors, parent, nameToOldChildren) {
            ...
            for (var i = 0, n = descriptors.length; i < n; ++i) {
                ...
                appendResourceStateDescriptors.call(
                    this,
                    descriptor.values,
                    childNode,
        ...
        var shouldHighlightChanges =
            this._resourceKindId(this._currentResourceId) ===
            this._resourceKindId(resourceState.id);
    ...
    _saveExpandedState: function() {
        if (!this._currentResourceId) return;
        var expandedState = {};
        ...
        function callback(node, key) {
            if (node.expanded) expandedState[key] = true;
        }
    ...
    _restoreExpandedState: function() {
        ...
        function callback(node, key) {
            if (expandedState[key]) node.expand();
        }
        this._forEachGridNode(callback);
    },
    ...
    _createDataGridNode: function(descriptor) {
        ...
        var valueElement = callArgument
            ? WebInspector.CanvasProfileDataGridHelper.createCallArgumentElement(callArgument)
            : '';
        var nameElement = name;
        if (typeof descriptor.enumValueForName !== 'undefined')
            nameElement = WebInspector.CanvasProfileDataGridHelper.createEnumValueElement(
                name,
                +descriptor.enumValueForName
            );
...
WebInspector.PieChart.prototype = {
    ...
    setSize: function(value) {
        ...
        if (this._totalElement) this._totalElement.style.lineHeight = size;
    },
    addSlice: function(value, color) {
        ...
        path.setAttribute(
            'd',
            'M0,0 L' + x1 + ',' + y1 + ' A1,1,0,' + largeArc + ',1,' + x2 + ',' + y2 + ' Z'
        );
