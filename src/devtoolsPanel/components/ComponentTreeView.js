        ...
        if (this.state.nodesMap.has(searchTerm)) {
            ...
            this.state.nodesMap.forEach(item => {
                if (
                    ...
                    (item.state.descriptor.startsWith(searchTerm) ||
                        item.state.descriptor.startsWith('markup://' + searchTerm))
                ) {
...
class ComponentTreeViewRootNode extends React.Component {
    ...
    render() {
        ...
        return <ul className="tree-view collapsable">{nodes}</ul>;
    }
}
...
class DomTreeViewNode extends React.Component {
    ...
    render() {
        if (Array.isArray(this.state.childNodes) && this.state.childNodes.length === 0) {
            return (
                <li
                    key={this.props.globalId}
                    className={this.state.selected ? 'tree-node-selected' : ''}
                >
                    <span className="tree-view-node" onClick={this.handleClickEvent}>
                        {this.props.label}
                    </span>
                </li>
            );
        ...
        return (
            <li
                className={
                    'tree-view-parent' +
                    (!this.state.collapsed ? ' tree-view-expanded' : '') +
                    (this.state.selected ? ' tree-node-selected' : '')
                }
                key={this.props.globalId}
            >
...
class ComponentTreeViewNode extends React.Component {
    ...
    getLabel() {
        ...
        if (this.props.showGlobalId) {
            attributes.push(
                <span className="component-attribute-pair" key="globalId">
                    <span className="component-attribute">globalId</span>="{this.props.globalId}"
                </span>
            );
        }
            ...
            for (var attribute in component.attributes) {
                ...
                if (current && Array.isArray(current)) {
                    ...
                    attributes.push(
                        <span className="component-attribute-pair" key={attribute}>
                            <span className="component-attribute">{attribute}</span>=
                            {FunctionCallValueUtils.formatFCV(current)}
                        </span>
                    );
                    ...
                    if (FunctionCallValueUtils.isFCV(current)) {
                        ...
                        attributes.push(
                            <span className="component-attribute-pair" key={attribute}>
                                <span className="component-attribute">{attribute}</span>="{current}"
                            </span>
                        );
                    }
                } else {
    ...
    update() {
        ComponentTreeSerializer.getComponentWithChildren(this.props.globalId).then(result => {
...
class HtmlTreeViewLabel extends React.Component {
    ...
    render() {
        ...
        const tagName =
            component.tagName || (component.attributes && component.attributes.tag) || 'wtf';
        const attributes = [];

        if (this.props.showGlobalId) {
            attributes.push(
                <span className="component-attribute-pair" key="globalId">
                    <span className="component-attribute">globalId</span>="{component.globalId}"
                </span>
            );
        }
        ...
        if (localId) {
            attributes.push(
                <span className="component-attribute-pair" key="aura:id">
                    <span className="component-attribute">aura:id</span>="{localId}"
                </span>
            );
        }
...
class TextTreeViewLabel extends React.Component {
    ...
    render() {
        ...
        return <span key={this.props.component.globalId}>"{text}"</span>;
    }
}
        ...
        if (this.props.component.expressions) {
            ...
            if (expression) {
                if (FunctionCallValueUtils.isFCV(expression)) {
                    return (
                        <span key={this.props.globalId}>
                            {FunctionCallValueUtils.formatFCV(expression)}
                        </span>
                    );
                }
