...
class OptionBar extends React.Component {
    ...
    render() {
        ...
        optionTypes = optionTypes.map((optionType, i) => {
            ...
            const visibleKey =
                optionType.name || optionType.key.substring(optionType.key.indexOf('__') + 2); // TODO what if array?
            ...
            switch (method) {
                case 'select':
                    return (
                        <div
                            key={optionType.key}
                            className={`option ${method} ${optionType.hasOwnProperty('require') &&
                                'required'}`}
                        >
                            <div className="select-label">
                                {visibleKey}
                                <span title="Will be updated on page refresh">{warning}</span>:
                            </div>
                            <Select
                ...
                case 'checkbox':
                    return (
                        ...
                        >
                            <Checkbox
                                checked={!!headOptionValue}
                                onChange={e =>
                                    onOptionChange(namespace, optionType.key, e.target.checked)
                                }
                            />
        ...
        return (
            <div className="options" style={style}>
                {optionTypes.length === 0 ? (
                    <div className="no-options">This module does not have any required options</div>
                ) : (
                    optionTypes
                )}
            </div>
