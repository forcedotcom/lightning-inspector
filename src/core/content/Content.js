...
export default class Content {
  ...
  constructor(namespaces, processors) {
    ...
    Promise.all([sync.get(this.namespaces), local.get(this.namespaces)]).then(([syncStorage, localStorage]) => {
      ...
      if (WHITELIST.test(window.location.pathname) || /forceGatherers/.test(window.location.search)) {
        ...
        try {
          if (xhr.readyState == XMLHttpRequest.DONE) {
            if (200 <= xhr.status && xhr.status < 300) {
              resolve({ body: Array.from(new Uint8Array(xhr.response)), headers: xhr.getAllResponseHeaders() });
            } else {
              resolve({ error: true });
            }
          }
  ...
  _handleMessage(message, sender, respond) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      console.log(
        ...
        (message.args || []).join(','),
        ...String(!message.bucket || message.bucket === 'active' ? this.buckets.counter : message.bucket).color('#AE81FF'),
    ...
    if (message.text === 'ContentInvoke') {
      ...
      Promise.resolve(response).then(respond).catch(e => console.error(e));
    } else if (message.text === 'ContentCollectorInvoke') {
      const { args = [], method, bucket } = message;
      const collector = !bucket || bucket === 'active' ? this.buckets.active() : this.buckets.get(bucket);
      ...
      if (this.valid[namespace] === true) {
        postMessage('GathererInvoke', { namespace, method, args }, payload => {
          let { data, collectMethod = 'set', collected, processor: processorName } = JSON.parse(payload);
  ...
  initViewerListeners() {
    browser.runtime.onMessage.addListener((message, sender, respond) => this._handleMessage(message, 'viewer', respond));
  }

  initInjectionListeners() {
    onMessage('InjectionSupport', ({ namespace, supported }) => this.setSupported(namespace, supported));
    onMessage('ContentDirect', (message, respond) => this._handleMessage(message, 'injector', respond));
  }
  ...
  performLocalStorageInjection() {
    injectScriptByText(`
       window.__injection_local_storage__ = window.__injection_local_storage__ ?
          Object.assign(window.__injection_local_storage__, ${JSON.stringify(this.localStorage)}) : {};
    `);
  }
}
