...
if (window.location.search.indexOf('syncStorage=') > -1) {
    try {
        ...
        const urlSyncStorage = JSON.parse(
            decodeURIComponent(getParam('syncStorage', window.location.href))
        );
        for (const namespace of urlSyncStorage) {
            syncStorage[namespace] = Object.assign(
                syncStorage[namespace] || {},
                urlSyncStorage[namespace]
            );
        }
    } catch (e) {
        // ignore
...
if (window.location.search.indexOf('localStorage=') > -1) {
    try {
        ...
        const urlLocalStorage = JSON.parse(
            decodeURIComponent(getParam('localStorage', window.location.href))
        );
        for (const namespace of urlLocalStorage) {
            localStorage[namespace] = Object.assign(
                localStorage[namespace] || {},
                urlLocalStorage[namespace]
            );
        }
    } catch (e) {
        // ignore
...
export default class Gatherer {
    ...
    markStart(event, id) {
        ...
        if (start) {
            if (collect) {
                ...
                marks[this.marksIdx++] = `${event};{"event":"${event}","cycle":"${
                    this.cycle
                }","namespace":"${this.namespace}","id":"${event + id}","ts":${((performance.now() *
                    1000) >>
                    0) /
                    1000},"type":"start"}`;
            }
    ...
    markEnd(event, id, name) {
        ...
        if (collect) {
            ...
            marks[this.marksIdx++] = `${event};{"event":"${event}","cycle":"${
                this.cycle
            }","name":"${name}","namespace":"${this.namespace}","id":"${event +
                id}","ts":${((performance.now() * 1000) >> 0) / 1000},"type":"end"}`;
        }
        ...
        if (collect) {
            ...
            if (typeof context === 'object') {
                marks[this.marksIdx++] = `${event};{"event":"${event}","cycle":"${
                    ...
                    id}","ts":${((performance.now() * 1000) >> 0) /
                    1000},"context":${JSON.stringify(context)},"type":"end"}`;
            } else {
                marks[this.marksIdx++] = `${event};{"event":"${event}","cycle":"${
                    this.cycle
                }","name":"${name}","namespace":"${this.namespace}","id":"${event +
                    id}","ts":${((performance.now() * 1000) >> 0) /
                    1000},"context":"${context}","type":"end"}`;
            }
        }
    ...
    markWithContext(event, id, name, context) {
        ...
        if (collect) {
            ...
            marks[this.marksIdx++] = `${event};{"event":"${event}","cycle":"${
                ...
                1000) >>
                0) /
                1000},"context":"${context}","type":"stamp"}`;
        }
    ...
    store(event, context) {
        if (typeof data === 'string') {
            this.datastore.push(
                `${event};{"event":"${event}","namespace":"${this.namespace}","context":"${context}","cycle":${this.cycle}}`
            );
        } else {
    ...
    cycleEnd() {
        if (this.cycleStarted === true) {
            this.markEndWithContext(
                'cycle',
                this.cycle,
                this.cycle,
                `initiatorEvent=${this.cycleInitiatorEvent}`
            );
            this.cycle++;
            this.cycleStarted = false;
            this.cycleInitiatorEvent = null;
        ...
        onMessage('GathererInvoke', (payload, respond) => {
            try {
                ...
                if (namespace === 'static') {
                    ...
                    Promise.all(responses.map(response => Promise.resolve(response))).then(data =>
                        respond(JSONfn.stringify(data))
                    );
                } else {
    ...
    static getMatcher(events) {
        return a =>
            typeof a === 'object'
                ...
                : events.includes(a.substring(0, a.indexOf(';')));
    }
