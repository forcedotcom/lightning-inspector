...
export default class EventParameters extends React.PureComponent {
        ...
        return Object.entries(parameters).map(parameter => {
            return (
                <Row key={parameter[0]}>
                    ...
                    <Cell data-label="Value">
                        <span>{this.formatValue(parameter[1])}</span>
                    </Cell>
                </Row>
            );
        });
    ...
    render() {
        return (
            <Table flavor={['bordered', 'striped']} variation="no-row-hover">
                <thead>
                    <Row head>
                        <Cell scope="col">Parameter</Cell>
                        <Cell scope="col">Value</Cell>
                    </Row>
                </thead>
