...
function isOverflowingElement(element) {
    ...
    const value = 'visible';
    return (
        style.getPropertyValue('overflow-x') !== value &&
...
function getScrollableParentRect(element) {
    ...
    const scrollingContainers = getParents(element)
        .slice(1)
    ...
    return scrollingContainers.reduce((previous, current) => {
...
function isCSSVisible(element) {
    ...
    for (let i = 0; i < parents.length; i++) {
        const style = window.getComputedStyle(parents[i]);
...
export default function getElementVisibleArea(element) {
    // https://github.com/medialize/ally.js/blob/master/src/util/visible-area.js

    if (element instanceof Element) {
        const visible = getVisible(element);
        return getVisible(element);
    } else if (element.parentElement instanceof Element) {
        return getVisible(element.parentElement);
    }
