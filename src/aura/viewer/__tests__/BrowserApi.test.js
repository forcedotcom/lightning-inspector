...
test("eval uses browser as the default browser Api", async () => {
 ...
 test("eval uses browser before chrome as the browser api", async () => {
    const expected = "var statement;";
    global.browser = mockBrowser();
    global.browser.devtools.inspectedWindow.eval = (command, callback) => { callback(expected); }
...
test("Eval exception in browser causes exception thrown in browser API", async () => {
    const expected = "var statement;";
    global.browser = mockBrowser();
    global.browser.devtools.inspectedWindow.eval = (command, callback) => { callback(expected, new Error("Expected Error Thrown")); }
...
test("getLabel uses i18n API to retrieve localized labels", () => {
    ...
    global.browser = mockBrowser();
    global.browser.i18n.getMessage = () => { return expected; };

    const actual = BrowserApi.getLabel("label_data");
...
function mockBrowser() {
    return {
        devtools: {
            inspectedWindow: {
                eval: (command, callback) => { callback(command); }
            }
        },
        i18n: {
            getMessage: () => {}
