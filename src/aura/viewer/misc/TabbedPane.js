...
class TabbedPane extends React.Component {
  ...
  setActiveTab(tab) {
    ...
    if (index > -1) {
      ...
      setTransform(ReactDOM.findDOMNode(this.refs.pane), `translate3d(-${index / this.props.tabs.length * 100}%, 0, 0)`);
  ...
  render() {
    ...
    const onClick = tab => typeof onSelectTab === 'function' ? onSelectTab(tab) : this.setActiveTab(tab);
    ...
    return (
      <div className='full flex vertical'>
        <div className={`tabs ${this.props.spread ? 'spread flex' : 'scroll scroll-x'}`}
             style={{ whiteSpace: 'nowrap', display: this.props.hideTabsOnOne && tabs.length === 1 ? 'none' : '' }}>
          {showClose ? <div className='tab tab-close' onClick={onClose}>✕</div> : null }
          {tabs}
        </div>
        <div className='full' style={style}>
          <div className='full-abs animate'
               ref='pane'
