...
class FixedDataTable extends React.PureComponent {
  render() {
    const { containerWidth, containerHeight, offsetWidth = 0, offsetHeight = 0, ...props } = this.props;

    return (
      <Table width={containerWidth + offsetWidth}
...
export class Text extends React.PureComponent {
  render() {
    let { rowIndex, columnKey, getColumnValue, isComparisonMode, append, align, className = '', ellipses, children, title, ...props } = this.props;
...
export class Header extends React.PureComponent {
  render() {
    let { columnKey, displayName = columnKey, onHeaderClick, sortDir, sortColumn, align, ellipses, style, className = '', ...props } = this.props;
    ...
    const columns = this.props.columns.map((columnProps, i) =>
      <Column
        ...
        header={<Header sortColumn={sortColumn}
                        ...
                        {...(columnProps.header || {})}/>}
        cell={<Text getColumnValue={(rowIndex, columnKey) => getColumnValue(rows, rowIndex, columnKey)}
                    {...(columnProps.body || {})}/>}
        width={columnWidths[columnProps.columnKey] || columnProps.width || 50}/>
    );

    return (
      <_Table rowsCount={rows.length}
