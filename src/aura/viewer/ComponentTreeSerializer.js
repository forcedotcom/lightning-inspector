...
function getComponent(globalId, callback, configuration) {
    if (typeof callback !== 'function') {
        throw new Error(
            'callback is required for - getComponent(globalId, callback)'
        );
    }
...
function getComponents(globalIds, callback) {
    if (!Array.isArray(globalIds)) {
        throw new Error(
            'globalIds was not an array - getComponents(globalIds, callback)'
        );
    }
...
function getBodyFromComponent(component, callback = () => {}) {
    ...
    if (body) {
        ...
        var currentId =
            (component.supers
                ? component.supers.reverse()[0]
                : component.globalId) || component.globalId;
        ...
        isExpression(component) &&
        isFacets(component.attributes.value)
    ) {
